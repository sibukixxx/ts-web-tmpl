...
export class TypeSafeRouter {
  ...
  private middlewares: Array<
    (context: MiddlewareContext, next: () => Promise<boolean>) => Promise<boolean>
  > = []
  ...
  >(section: Section, route: Route, query?: QueryParams): Promise<boolean> {
    try {
  ...
  private async navigateToPathWithContext(context: MiddlewareContext): Promise<boolean> {
    try {
      this.setLoading(true)
  ...
  // ユーティリティメソッド
  private generatePath(pattern: RoutePattern | string, params: Record<string, string>): string {
    return pattern
      .split('/')
      .map((segment) => {
  ...
  private setLoading(loading: boolean): void {
    ...
    this.navigationListeners.forEach((listener) => listener(this.getCurrentPath()))
  }
